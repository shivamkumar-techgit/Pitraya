...
import { ShieldCheck, Lock, Mail, Eye, EyeOff, KeyRound, AlertCircle, Loader2, Sparkles } from "lucide-react";

export interface AdminLoginFormProps extends React.HTMLAttributes<HTMLDivElement> {
  onSuccess?: () => void;
}
...
export default function AdminLoginForm({ onSuccess, className, ...props }: AdminLoginFormProps) {
  ...
  return (
    <div
      className={`min-h-screen flex items-center justify-center px-4 py-8 md:py-12 bg-gradient-to-b from-neutral-950 via-neutral-900 to-neutral-950 text-neutral-100 ${className || ""}`}
      {...props}
    >
      <motion.div
      ...
      >
        ...
        <div className="text-center space-y-3">
          <div className="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-amber-500/10 border border-amber-500/20 shadow-lg shadow-amber-500/5 mb-2">
            <ShieldCheck className="w-8 h-8 text-amber-400" />
          </div>
          <h1 className="text-2xl md:text-3xl font-bold tracking-tight font-serif text-amber-100">
            Pitraya Admin Portal
          </h1>
          <p className="text-sm text-neutral-400 max-w-xs mx-auto">
            Sacred Concierge Operations & Lineage Management Dashboard
          </p>
        </div>
        ...
        <div className="p-6 md:p-8 rounded-2xl border border-neutral-800 bg-neutral-900/80 backdrop-blur-xl shadow-xl space-y-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-1.5">
              <label htmlFor="email-input" className="block text-xs font-semibold text-neutral-300 uppercase tracking-wider">
                Admin Email
              </label>
            ...
            {/* Password Field */}
            <div className="space-y-1.5">
              <label htmlFor="password-input" className="block text-xs font-semibold text-neutral-300 uppercase tracking-wider">
                Password
              </label>
              <div className="relative">
          ...
          {process.env.NODE_ENV === "development" && (
            <div className="pt-4 border-t border-neutral-800 space-y-3">
              <div className="flex items-center justify-between text-xs text-neutral-400">
                ...
                <span className="text-neutral-500">Select role to fill</span>
              </div>
        ...
        <p className="text-center text-xs text-neutral-500">
          Protected by Session JWT & Bcrypt Role Authorization
        </p>
      </motion.div>
    </div>
  );
