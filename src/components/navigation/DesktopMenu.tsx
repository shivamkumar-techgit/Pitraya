...
export default function DesktopMenu({
  ...
  return (
    <nav
      ...
      className={cn("flex items-center gap-1 lg:gap-1.5 xl:gap-2.5 2xl:gap-4 flex-nowrap", className)}
    ...
    >
      {items.map((item, idx) => {
        ...
        <div
            key={idx}
            className={cn(
              "relative py-2 group shrink-0",
              // Hide lower-priority items below xl to prevent 1366px overflow
              (item.label === "Sanctuaries" || item.label === "AI Planner") && "hidden xl:block"
            )}
          ...
          >
            <Link
              ...
              className={cn(
                "inline-flex items-center gap-1 text-[10.5px] xl:text-[11.5px] 2xl:text-xs font-semibold tracking-wider uppercase transition-colors duration-200 select-none whitespace-nowrap rounded-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gold-primary",
                isActive
            ...
            {isActive && (
              <motion.span
                ...
                className="absolute bottom-1.5 left-0 right-0 h-[1.5px] bg-gold-primary rounded-full shadow-[0_0_8px_rgba(168,117,5,0.6)]"
